...
function buildVents(bodyW: number, _bodyH: number, bodyDepth: number) {
  ...
  const totalH = 4 * slotH + 3 * gap;
  const startY = totalH * 0.5;

  const fillMat = new THREE.MeshBasicMaterial({ color: 0x010101, side: THREE.DoubleSide });
  ...
  function stadium(w: number, h: number) {
    const r = w * 0.5;
  ...
  function stadiumRing(w: number, h: number, t: number) {
    const o = stadium(w + t, h + t);
    const i = stadium(w, h);
    o.holes.push(i);
    return o;
  }

  function addSlot(cy: number, h: number, zOff = 0) {
    const mesh = new THREE.Mesh(new THREE.ShapeGeometry(stadium(slotW, h)), fillMat);
    mesh.rotation.y = -Math.PI * 0.5;
    mesh.position.set(sideX, cy, ventZ + zOff);
    mesh.renderOrder = 8;
    vents.add(mesh);

    const ring = new THREE.Mesh(new THREE.ShapeGeometry(stadiumRing(slotW, h, 0.006)), rimMat);
    ring.rotation.y = -Math.PI * 0.5;
    ring.position.set(sideX - 0.001, cy, ventZ + zOff);
    ring.renderOrder = 8;
    vents.add(ring);
  }

  for (let i = 0; i < 4; i++) {
    const cy = startY - i * (slotH + gap) - slotH * 0.5;
    addSlot(cy, slotH);
  }

  const compY = startY - 4 * (slotH + gap) - slotH * 0.5;
  addSlot(compY, slotH);

  const sqSize = 0.035;
  const sqR = 0.010;
  const sqEx = 3.2;
  const sqPts = squircleRectPoints({
  ...
  sqShape.moveTo(sqPts[0].x, sqPts[0].y);
  sqPts.slice(1).forEach((p) => sqShape.lineTo(p.x, p.y));
  sqShape.closePath();

  const sqMesh = new THREE.Mesh(new THREE.ShapeGeometry(sqShape), fillMat);
  sqMesh.rotation.y = -Math.PI * 0.5;
  sqMesh.position.set(sideX, compY, ventZ + 0.022);
  sqMesh.renderOrder = 8;
  vents.add(sqMesh);

  const sqOuterPts = squircleRectPoints({
  ...
  sqRingShape.moveTo(sqOuterPts[0].x, sqOuterPts[0].y);
  sqOuterPts.slice(1).forEach((p) => sqRingShape.lineTo(p.x, p.y));
  sqRingShape.closePath();
  sqRingShape.holes.push(sqShape);

  const sqRing = new THREE.Mesh(new THREE.ShapeGeometry(sqRingShape), rimMat);
